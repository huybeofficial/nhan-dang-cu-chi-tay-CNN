...
def predict_rgb_image(image):
    ...
    acc = float("%0.2f" % (max(pred_array[0]) * 100))
    print(result)
    return result, acc
...
while camera.isOpened():
    ...
    if isBgCaptured == 1:
        ...
        cv2.imshow('thresh', cv2.resize(thresh, dsize=None, fx=1, fy=1))
        ...
        if (np.count_nonzero(thresh)/(thresh.shape[0]*thresh.shape[0])>0.2):
            ...
            if (thresh is not None):
                ...
                prediction, acc = predict_rgb_image(target)
                ...
                if (acc>=80):
                    cv2.putText(frame, prediction, (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 3,
                                (225,0 , 0), 10, lineType=cv2.LINE_AA)
    thresh = None
